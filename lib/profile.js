...
exports.deleteAccount = function (request, reply) {
  if (request.session.get('op') || request.session.get('uid') === request.payload.uid) {
    // delete posts and account

    posts.getAllByUser(request.payload.uid, function (err, all) {
      if (err) {
        return reply(Boom.wrap(err, 500));
      }

      exports.getByUID(request.payload.uid, function (err, user) {
        if (err) {
          return reply(Boom.wrap(err, 500));
        }
        ...
        deletePostsAndUser(all, user, function (err, result) {
          ...
          if (request.session.get('uid') === request.payload.uid) {
