...
// This converts U.S./Canadian numbers to be without an area code and removes the + for
// others for consistency.
var cleanNumber = function (phone) {
  phone = phone.replace('+', '');

  if (phone.match(/^1/) && phone.length === 11) {
...
var register = function (request, reply) {
  ...
  var phone = cleanNumber(request.session.get('phone'));
...
exports.login = function (request, reply) {
  ...
  var generate = function () {
    utils.generatePin(phone, function (err) {
      ...
      request.session.set('phone', cleanNumber(request.payload.phone));
